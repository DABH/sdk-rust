...
// Walks the inbound activity interceptor chain
pub(crate) fn call_execute_activity(
    interceptors: Arc<[Arc<dyn ActivityInboundInterceptor>]>,
    input: ExecuteActivityInput,
    next: Next<'static, ExecuteActivityInput, ExecuteActivityOutput<'static>>,
) -> ExecuteActivityOutput<'static> {
    call_execute_activity_inner(interceptors, 0, input, next)
}

fn call_execute_activity_inner(
    interceptors: Arc<[Arc<dyn ActivityInboundInterceptor>]>,
    index: usize,
    input: ExecuteActivityInput,
    next: Next<'static, ExecuteActivityInput, ExecuteActivityOutput<'static>>,
) -> ExecuteActivityOutput<'static> {
    let Some(interceptor) = interceptors.get(index).cloned() else {
        return next.run(input);
    };
    let next_interceptors = interceptors.clone();
    let next_index = index + 1;
    Box::pin(async move {
        let next_step = Next::new(move |input| {
            call_execute_activity_inner(next_interceptors, next_index, input, next)
        });
        interceptor.execute_activity(input, next_step).await
    })
}
